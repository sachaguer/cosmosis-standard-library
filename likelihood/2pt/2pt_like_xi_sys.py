...
TWOPCF_TYPES = [twopoint.Types.galaxy_shear_plus_real, twopoint.Types.galaxy_shear_minus_real]
...
class TwoPointLikelihood(GaussianLikelihood):
    ...
    def __init__(self, options):
        ...
        self.add_xi_sys = options.get_bool('add_xi_sys', default=False)
    ...
    def extract_theory_points(self, block):
        ...
        for ii, spectrum in enumerate(self.two_point_data.spectra):
            ...
            #Add a contribution from PSF systematics parameters
            if self.add_xi_sys and spectrum.type1 in TWOPCF_TYPES and spectrum.type2 in TWOPCF_TYPES:
                section, x_name, y_name = theory_names(spectrum)
                
                xi_sys = block['xi_sys', section]
                theta = block['xi_sys', 'theta']
                index = np.searchsorted(theta, angle_vector)
                theory_vector = theory_vector + xi_sys[index]
